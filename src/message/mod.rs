...
impl<'a> MessageFormatter<'a> {
    ...
    fn push_in_reply(
        ...
        let (mut replied, proto) = msg.show_msg(w, style, true, settings, info, previews);
...
impl Message {
    ...
    pub fn show_with_preview<'a>(
        ...
        match &reply {
            Some(Some(r)) => {
                let proto = fmt.push_in_reply(r, style, &mut text, info, settings, previews);
        ...
        let (msg, proto) = self.show_msg(width, style, true, settings, info, previews);
