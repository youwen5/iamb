                            ...
                            if let Some(stem) = filename.file_stem().and_then(OsStr::to_str) {
                                ...
                                for n in 1..=1000 {
                                    if let Some(ext) = ext.and_then(OsStr::to_str) {
                                        filename_incr
                                            .set_file_name(format!("{}-{}.{}", stem, n, ext));
                                    } else {
            ...
            SendAction::Submit | SendAction::SubmitFromEditor => {
                ...
                if let Some((_, event_id)) = &self.editing {
                    ...
                    if let Some(message) = info.get_event(event_id) {
                        if let MessageEvent::Original(ev, _) = &message.event {
...
impl Editable<ProgramContext, ProgramStore, IambInfo> for ChatState {
    fn editor_command(
        ...
        let info = store.application.rooms.get_or_default(self.room_id.clone());
        let adjusted = self.auto_toggle_focus(act, ctx, info);
...
pub fn auto_toggle_focus(
    ...
    reply_to: &mut Option<MessageKey>,
    info: &RoomInfo,
) -> Option<EditorAction> {
    ...
    match (focus, act) {
        (f @ RoomFocus::Scrollback, _) if is_insert => {
            ...
            if reply_to.is_none() && !tbox.get_text().is_empty() {
                *reply_to = scrollback.get_key(info);
            }
            f.toggle();
            None
        },
