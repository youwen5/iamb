...
use crate::base::{ChatStore, IambBufferId, IambId, IambInfo, RoomFocus, MATRIX_ID_WORD};
...
/// Tab completion for `:unreact`
fn complete_iamb_unreact(args: Vec<String>, store: &ChatStore) -> Vec<String> {
    if args.len() != 1 {
        return vec![];
    }
    let (IambId::Room(room_id, _), Some(event_id)) = store.last_focus.clone() else {
        return vec![];
    };

    let Some(reactions) = store.rooms.get(&room_id).and_then(|info| info.reactions.get(&event_id))
    else {
        return vec![];
    };
    ...
    for (reaction, user_id, _) in reactions.values() {
        if user_id != &store.settings.profile.user_id {
            continue;
        }
        if reaction.starts_with(input) {
            res.push(reaction.clone());
        }
        if let Some(shortcode) = emojis::get(reaction).and_then(|emoji| emoji.shortcode()) {
            if shortcode.starts_with(input) {
                res.push(shortcode.to_owned())
            }
        }
    }
...
fn complete_cmdarg(
    ...
    *cursor = new_cursor;
    ...
    let completions = match cmd.name.as_str() {
        ...
        "unreact" => complete_iamb_unreact(args, store),
    ...
    match id.chars().next() {
        ...
        Some('@') | None => {
            ...
            let mut users: HashSet<_> = info
                ...
                .into_iter()
                .flat_map(|n| {
                    info.display_name_completion
                        .get(&n)
                        .unwrap()
                        .iter()
                        .map(move |id| format!("[{}]({})", n, id.matrix_to_uri()))
                })
                .collect();
            ...
            users.extend(info.display_names.complete(id.as_ref()).into_iter().map(|i| {
                format!(
                    "[{}]({})",
                    info.display_names.get(&i).map(|(n, _)| n).unwrap_or(&i.to_string()),
                    i.matrix_to_uri()
                )
            }));
