...
use matrix_sdk::ruma::{OwnedMxcUri, OwnedRoomAliasId};
use ratatui::{
    buffer::Buffer,
...
pub struct RoomInfo {
    ...
    pub display_names: CompletionMap<OwnedUserId, (String, bool)>,

    /// Tab completion for the display names in this room.
    pub display_name_completion: CompletionMap<String, SmallVec<[OwnedUserId; 1]>>,
    ...
    pub fn unreads(&self, settings: &ApplicationSettings) -> UnreadInfo {
        ...
        let last_receipt = last_receipt.as_ref().and_then(|event_id| {
            match &self.keys.get(*event_id)? {
                ...
                EventLocation::Edit(_, key) => Some(key),
